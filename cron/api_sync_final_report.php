...
if (!file_exists($configPath)) $configPath = CRON_ROOT . '/config/cron_config.sample.php';
...
$defaultDateTo = date('Y-m-d 23:59:59', strtotime("-1 day"));
$defaultDateFrom = date('Y-m-d 00:00:00', strtotime("-{$daysBack} days"));
...
try {
    ...
    if (!empty($dateRow['date_from']) && !empty($dateRow['date_to'])) {
        $dateFrom = $dateRow['date_from'];
        $dateTo = $dateRow['date_to'];
    ...
    if ($tokenParam) $params[$tokenParam] = $bearerToken;
    $url = $apiBaseUrl . '?' . http_build_query($params);
    out("Calling API...");

    $headers = ['Accept: application/json'];
    if (!$tokenParam) $headers[] = 'Authorization: Bearer ' . $bearerToken;
    $ch = curl_init($url);
